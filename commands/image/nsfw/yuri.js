...
class yuricommands extends Command {
  ...
  async exec(message) {
    ...
    const embed = new MessageEmbed()
      .setTitle("L'image ne s'affiche pas clique ici !").setURL(`${yuri}`)
      .setImage(yuri)
      .setFooter(`Demandé par ${message.author.username}`)
      .setTimestamp();
    message.channel.send(embed).then(console.log(`La commande yuri a été exécuté par ${message.author.tag} de l'id : ${message.author}`));
  }
}
