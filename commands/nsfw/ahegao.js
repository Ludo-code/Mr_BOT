...
export const command = {
    ...
    async execute(message, args) {
        try {
            let res = await (await fetch(`https://api.klipy.com/api/v1/${process.env.KLIPY_API_KEY}/gifs/search?customer_id=${process.env.KLIPY_API_KEY_NAME}&limit=24&q=ahegao`))?.json();
