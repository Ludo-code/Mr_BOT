...
class piedgifcommands extends Command {
  ...
  async exec(message) {
    ...
    const embed = new MessageEmbed()
      .setTitle("L'image ne s'affiche pas clique ici !").setURL(`${piedgif}`)
      .setImage(piedgif)
      .setFooter(`Demandé par ${message.author.username}`)
      .setTimestamp();
    message.channel.send(embed).then(console.log(`La commande pied_gif a été exécuté par ${message.author.tag} de l'id : ${message.author}`));
  }
}
