...
class rdmhentaicommands extends Command {
  ...
  async exec(message) {
    ...
    const rdmhentaigif = await fetch("https://nekos.life/api/v2/img/Random_hentai_gif")
      .then(res => res.json())
      .then(json => json.url);

    const embed = new MessageEmbed()
      .setTitle("L'image ne s'affiche pas clique ici !").setURL(`${rdmhentaigif}`)
      .setImage(rdmhentaigif)
      .setFooter(`Demandé par ${message.author.username}`)
      .setTimestamp();
    message.channel.send(embed).then(console.log(`La commande hentai_gif a été exécuté par ${message.author.tag} de l'id : ${message.author}`));
  }
}
